...
from utils.vision.seg_gpt.engine import inference_image, inference_video
import utils.vision.seg_gpt.model as model
...
def prepare_model(chkpt_dir, arch='seggpt_vit_large_patch16_input896x448', seg_type='instance'):
    # build model
    model: torch.Module = getattr(model, arch)()
...
if __name__ == '__main__':
    ...
    if args.input_image is not None:
        ...
        print(f"Time to run: {delta_time / 1_000_000} ms to run")
